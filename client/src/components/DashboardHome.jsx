import { useMemo } from "react";
import {
  chartUsers,
  chartConversion,
  chartProduct,
  chartRevenue,
  barChartRevenue,
  barChartVisit,
} from "../utils/data";
import { TopBox, LineCharts, ChartBar, ChartPie, ChartArea } from "./chartPage";

function DashboardHome() {
  return (
    <div className="grid xl:gap-5 lg:gap-4 gap-3 grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 auto-rows-[minmax(160px,auto)] grid-flow-dense mt-7">
      <div className="p-[14px] rounded-xl border-2 border-slate-400 col-span-1 row-span-3">
        <TopBox />
      </div>
      <div className="p-[14px] rounded-xl border-2 border-slate-400 col-span-1">
        <LineCharts {...chartUsers} />
      </div>
      <div className="p-[14px] rounded-xl border-2 border-slate-400 col-span-1">
        <LineCharts {...chartRevenue} />
      </div>
      <div className="p-[14px] rounded-xl border-2 border-slate-400  col-span-1 row-span-3">
        <ChartPie />
      </div>
      <div className="p-[14px] rounded-xl border-2 border-slate-400 col-span-1">
        <LineCharts {...chartProduct} />
      </div>
      <div className="p-[14px] rounded-xl border-2 border-slate-400 col-span-1">
        <LineCharts {...chartConversion} />
      </div>
      <div className="p-[14px] rounded-xl border-2 border-slate-400 col-span-1 md:col-span-2 row-span-2">
        <ChartArea />
      </div>
      <div className="p-[14px] rounded-xl border-2 border-slate-400 col-span-1">
        <ChartBar {...barChartRevenue} />
      </div>
      <div className="p-[14px] rounded-xl border-2 border-slate-400 col-span-1">
        <ChartBar {...barChartVisit} />
      </div>
    </div>
  );
}

export default DashboardHome;
